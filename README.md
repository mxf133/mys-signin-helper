...
> **v2.2.2** · [更新日志](#更新日志) · **代码、文档与图标均由 AI 生成（vibe coding）**
...
**修复：图标在小尺寸下显示成一片破碎的色块。** `icon.ico` 里 16/24/32/48/64
五个位图帧的 `BITMAPINFOHEADER` 把 `biHeight` 写成了图像高度（ICO 规范要求写
**2 倍**：上半彩色位图 + 下半 AND 掩码），而且完全没有 AND 掩码。
Windows 的图标加载器固定按 `biHeight / 2` 解析，这几个档就被解成了半高，
渲染出来是乱码色块——桌面快捷方式和资源管理器里看到的都是这个样子。
只有 128/256 两个 PNG 档正常。现在按规范重新生成，尺寸档也扩到 10 档
（16/20/24/32/40/48/64/96/128 走 DIB + 256 走 PNG）。

界面与功能跟 v2.2.1 完全一致，只换了图标。

...
**修复：v2.2.0 里整个「操作」面板不见了。** 加「勾选要签到的游戏」工具条时，
操作区和日志区被放到了同一个 grid 单元格（都是 `row=3`）——Tk 遇到同格会直接
叠加，后放的日志区把操作区整个盖住了，所以【一键全部签到】【刷新状态】
【登录米游社】【清除登录凭据】和自动签到时间那一行都消失了（功能其实是好的，
只是点不到）。

- 日志区挪到 `row=4`、状态栏 `row=5`，`rowconfigure` 的伸缩权重跟着改成第 4 行
- 顺带补了一组**布局回归测试**：断言根容器里没有两个控件挤在同一格、
  操作区与日志区不同行且不重叠、操作区按钮全部 `winfo_ismapped`。
  把日志区改回 `row=3` 复现原 bug，该测试能稳定报出 6 项 FAIL

> 如果你装的是 v2.2.0，升级到本版即可；v2.2.0 的签到与勾选功能本身没有问题。
